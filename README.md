...
主页面底栏中，“开始转写”左侧的 Chatbox 按钮会在底栏上方打开快速输入栏。默认点击发送后调用当前翻译 Provider，并将原文和译文一起发送；输入栏左侧可向上展开目标语言、发送内容、消息格式、译文编辑和 144 字符处理设置。发送成功以 UDP 实际写入为准，翻译或发送失败时保留当前草稿并显示原因。
