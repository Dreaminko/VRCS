import { Eye, RotateCcw, ShieldCheck, Sparkles } from "lucide-react";
  ...
  return (
    <div className="translation-config-row translation-enhancement-row">
      ...
      <div className="translation-enhancement-fields">
        ...
        {preview && (
          <div className="translation-prompt-preview">
        ...
        {translation.prompt.context_enabled && (
          <>
            <div className="translation-context-sources">
              {([
                ["include_speaker", "speaker"],
                ["include_microphone", "microphone"],
                ["include_chatbox", "chatbox"],
              ] as const).map(([field, label]) => (
                <label key={field}>
                  <input
                    type="checkbox"
                    checked={translation.prompt[field]}
                    disabled={disabled}
                    onChange={(event) => update({ [field]: event.target.checked })}
                  />
                  {t(`settings.translation.contextSources.${label}`)}
                </label>
              ))}
            </div>
