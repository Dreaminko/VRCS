...
pub(super) async fn statuses(State(state): State<Arc<AppState>>) -> Json<Value> {
    let config = current_config(&state);
    Json(json!(state.glossary.statuses(&config)))
}
...
pub(super) async fn refresh(
    ...
    if state.glossary.refresh(&id).await.map_err(refresh_error)? {
        reload_asr_context(&state).await;
    }
    let config = current_config(&state);
    Ok(Json(json!(state.glossary.statuses(&config))))
}

pub(super) async fn legacy_subscription_status(State(state): State<Arc<AppState>>) -> Json<Value> {
    let config = current_config(&state);
    Json(json!(state.glossary.legacy_status(&config)))
}
...
pub(super) async fn legacy_subscription_refresh(
    ...
    let config = current_config(&state);
    Ok(Json(json!(state.glossary.legacy_status(&config))))
}

async fn reload_asr_context(state: &Arc<AppState>) {
...
fn current_config(state: &AppState) -> GlossaryConfig {
    state.config.read().expect("config lock").glossary.clone()
}
