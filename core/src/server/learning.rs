...
pub(super) async fn learning_items(
    ...
    let items = db_call(Arc::clone(&state.content.db), move |db| {
...
pub(super) async fn learning_capture_keys(
    ...
    let keys = db_call(Arc::clone(&state.content.db), |db| {
        db.learning_capture_keys()
    })
    .await
    .map_err(learning_db_error)?;
...
pub(super) async fn learning_item_create(
    ...
    let item = db_call(Arc::clone(&state.content.db), move |db| {
...
pub(super) async fn learning_item_delete(
    ...
    validate_id(id)?;
    let deleted = db_call(Arc::clone(&state.content.db), move |db| {
        db.delete_learning_item(id)
    })
    .await
    .map_err(learning_db_error)?;
    if !deleted {
...
pub(super) async fn learning_item_export(
    ...
    let note_id = anki_service::create_card(&state.integrations.http, &card, &anki_config)
        .await
        .map_err(|error| {
            api_error_with_params(
